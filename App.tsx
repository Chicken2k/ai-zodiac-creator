...
import originalTemplates from "./Mau_Kich_Ban_12_Con_Giap.json";
...
const App: React.FC = () => {
  ...
  const [selectedDuration, setSelectedDuration] =
    useState<DurationType>("1.5 phút");
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      const cleanedQueue = parsedQueue.map((item: QueueItem) =>
        item.status === "pending"
          ? { ...item, status: "error", content: "Đã bị hủy do tải lại trang." }
          : item,
      );
            ...
            qItem.id === item.id
              ? {
                  ...
                  content:
                    typeof error === "string"
                      ? error
                      : "Lỗi kết nối thiên cơ hoặc đã dừng.",
                }
              : qItem,
          ),
        );
  ...
  const fillTopicWithZodiac = (
    topicTemplate: string,
    zodiac: Zodiac,
    product: string,
  ) => {
    const filledTopic = applyTemplateVariables(topicTemplate, zodiac, product);
      ...
      prev.map((item) =>
        item.id === id
          ? {
              ...item,
              status: "error",
              content: "Đã dừng quá trình luận giải theo ý con.",
            }
          : item,
      ),
    );
  ...
  const handleGenerate = () => {
    ...
    if (useRandomTemplate && currentTopics.length > 0) {
      ...
      finalTopic = fillTopicWithZodiac(
        randomTemplate,
        selectedZodiac,
        productInput,
      );
    ...
    const resolvedTopic = applyTemplateVariables(
      finalTopic,
      selectedZodiac,
      productInput,
    );
    if (resolvedTopic !== finalTopic) {
      setFocus(resolvedTopic);
    }
  ...
  const handleGenerateFromMaster = () => {
    // 1. Random kịch bản từ file gốc
    const templates = originalTemplates as string[];
    const randomTemplate =
      templates[Math.floor(Math.random() * templates.length)];

    // 2. Random tuổi (Zodiac)
    const randomZodiac = ZODIACS[Math.floor(Math.random() * ZODIACS.length)].id;

    // 3. Đổ dữ liệu vào template (Sử dụng randomZodiac và giữ nguyên product hiện tại)
    const filledTopic = applyTemplateVariables(
      randomTemplate,
      randomZodiac,
      productInput,
    );

    // Cập nhật UI để người dùng thấy tuổi được chọn ngẫu nhiên
    setSelectedZodiac(randomZodiac);
    setFocus(filledTopic);

    // 4. Thêm vào hàng đợi (Giữ nguyên tone, duration, product từ UI)
    addToQueue(
      randomZodiac,
      selectedTone,
      selectedDuration,
  ...
  const clearAll = (e?: React.MouseEvent) => {
    ...
    if (window.confirm("Con muốn xóa sạch lịch sử hàng đợi không?")) {
      Object.values(abortControllers.current).forEach(
  ...
  const handleDownloadTemplate = () => {
    const sampleData = [
      "Cái nết của [TUOI] là [TRAIT], nhưng [PAIN].",
      "[TUOI] đi với [RIVAL_1] hoặc [RIVAL_2] thì nên giữ lời ăn tiếng nói.",
      "Đường tiền [TUOI]: [MONEY]. Có duyên thì giữ [Product] bên mình.",
      "Năm [YEAR], tín hiệu của [TUOI] là [SIGNAL].",
      ...
      try {
        ...
        if (typeof result === "string") {
          ...
          if (
            ...
            alert(
              "File không đúng định dạng. Vui lòng tải file mẫu để xem cấu trúc chuẩn.",
            );
          }
        }
      } catch (error) {
  ...
  const handleResetTemplate = () => {
    if (confirm("Khôi phục về danh sách mẫu câu mặc định?")) {
      setCurrentTopics(originalTemplates as string[]);
    ...
    <div className="h-screen flex flex-col bg-[#f3f4f6] text-slate-700 font-sans text-[12px] overflow-hidden">
      ...
      <header className="bg-white border-b border-slate-200 px-4 py-2 shrink-0 z-30 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-2">
          ...
          <h1 className="font-serif font-bold text-red-900 text-sm tracking-tight">
            THẦY PHÁN AI
          </h1>
        </div>
                  ...
                  >
                    <div className="flex justify-between items-start mb-1">
                      <div className="flex items-center gap-1.5">
                        {item.status === "pending" && (
                          <ArrowPathIcon className="w-3 h-3 text-amber-500 animate-spin" />
                        )}
                        ...
                        <span
                          className={`text-[11px] font-bold ${activeItemId === item.id ? "text-red-900" : "text-slate-700"}`}
                        >
                          Tuổi {item.zodiac}
                        </span>
                      </div>
                    ...
                    <p className="text-[10px] text-slate-500 line-clamp-2 leading-relaxed pr-4 mb-1">
                      {item.focus.length > 60
                        ? item.focus.substring(0, 60) + "..."
                        : item.focus}
                    </p>
        ...
        <main className="flex-1 bg-[#f8f9fb] relative flex flex-col min-w-0">
          {activeItem ? (
              ...
              <div className="max-w-2xl mx-auto bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden min-h-[500px] flex flex-col">
                ...
                <div className="bg-slate-50/50 border-b border-slate-100 p-4 flex items-center justify-between sticky top-0 z-10 backdrop-blur-sm">
                  <div className="flex flex-col gap-1">
                    ...
                    {activeItem.product &&
                      activeItem.product !== "Không bán hàng" && (
                  ...
                  <div className="flex gap-2">
                    {activeItem.status === "pending" && (
                      <div className="flex items-center gap-2">
                        {isProcessing &&
                        queue.find((i) => i.status === "pending")?.id ===
                          activeItem.id ? (
                    ...
                    {activeItem.status === "completed" && (
                      <button
                        onClick={() =>
                          copyToClipboard(activeItem.content, activeItem.id)
                        }
                    ...
                    <div className="h-full flex flex-col items-center justify-center space-y-6">
                      ...
                      <div className="text-center space-y-2">
                        ...
                        queue.find((i) => i.status === "pending")?.id ===
                          activeItem.id ? (
                          <>
                            <h3 className="font-serif text-lg text-slate-700 italic">
                              Thầy đang chấm bút...
                            </h3>
                            <p className="text-slate-400 text-xs max-w-xs mx-auto">
                              Đang luận giải cho tuổi {activeItem.zodiac} với
                              chủ đề:
                              <br />
                              <span className="font-semibold text-slate-600">
                                "
                                {activeItem.focus.length > 50
                                  ? activeItem.focus.substring(0, 50) + "..."
                                  : activeItem.focus}
                                "
                              </span>
                            </p>
                          ...
                          <>
                            <h3 className="font-serif text-lg text-slate-500">
                              Đang xếp hàng chờ...
                            </h3>
                            <p className="text-slate-400 text-xs max-w-xs mx-auto">
                              Hệ thống đang xử lý các yêu cầu trước đó. Vui lòng
                              đợi trong giây lát.
                            </p>
                          </>
                        )}
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-slate-300 p-8">
              ...
              <h3 className="text-lg font-serif font-bold text-slate-400 mb-2">
                Chưa chọn nội dung
              </h3>
              <p className="text-[11px] max-w-xs text-center">
                Chọn một mục từ danh sách bên trái để xem chi tiết, hoặc tạo mới
                từ bảng điều khiển bên phải.
              </p>
            </div>
          )}
        </main>
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="mb-2 flex gap-1.5 overflow-x-auto pb-2 -mx-1 px-1 no-scrollbar">
                  {currentTopics.map((topic, index) => (
                    ...
                    >
                      {(() => {
                        const previewTopic = applyTemplateVariables(
                          topic,
                          selectedZodiac,
                          productInput,
                        );
                        return previewTopic.length > 50
                          ? previewTopic.substring(0, 50) + "..."
                          : previewTopic;
                      })()}
                    </button>
                  ))}
              ...
              <div className="pt-4 space-y-3 pb-6">
                ...
                <button
                  ...
                  className="w-full py-3.5 bg-amber-600 hover:bg-amber-700 text-white rounded-xl font-black text-xs flex items-center justify-center gap-2.5 transition-all active:scale-[0.98] shadow-lg shadow-amber-900/20 group"
                >
